...
(function () {
  ...
  var demo = (function () {
    ...
    return {
      ...
      orders: [
        { id: "o1", order_no: "ORD-0001", order_date: new Date().toISOString().slice(0,10), party_name: "Britannia", item_name: "Maida", order_qty: 500, scheduled_qty: 500, delivered_qty: 0, status: "Open", created_at: ago(180) },
        { id: "o2", order_no: "ORD-0002", order_date: new Date().toISOString().slice(0,10), party_name: "Britannia", item_name: "Maida", order_qty: 500, scheduled_qty: 500, delivered_qty: 0, status: "Open", created_at: ago(350) }
      ],
      schedules: [
      ...
      trips: [
        { id: "t1", trip_no: "TRIP-0001", schedule_id: "s1", order_id: "o1", vehicle_id: "v1", order_no: "ORD-0001", schedule_no: "SD-0001", vehicle_number: "AS01SC-6927", party_name: "Britannia", item_name: "Maida", qty: 500, current_stage: "LOADING_START", status: "Loading", planned_gate_entry: ago(120), actual_gate_entry: ago(113), actual_empty_weighment: ago(92), actual_loading_start: ago(42), actual_loading_complete: null, actual_loaded_weighment: null, actual_documentation: null, actual_gate_out: null, actual_party_store: null, actual_pod: null, created_at: ago(170), updated_at: ago(42) },
        { id: "t2", trip_no: "TRIP-0002", schedule_id: "s2", order_id: "o2", vehicle_id: "v2", order_no: "ORD-0002", schedule_no: "SD-0002", vehicle_number: "AS01QC-6569", party_name: "Britannia", item_name: "Maida", qty: 500, current_stage: "PARTY_STORE", status: "At Party", planned_gate_entry: ago(330), actual_gate_entry: ago(322), actual_empty_weighment: ago(307), actual_loading_start: ago(295), actual_loading_complete: ago(230), actual_loaded_weighment: ago(216), actual_documentation: ago(200), actual_gate_out: ago(180), actual_party_store: ago(15), actual_pod: null, created_at: ago(360), updated_at: ago(15) }
      ],
      events: [
  ...
  async function completeStage(trip, stage, actual, details, data) {
    ...
    ["empty_weight", "loaded_weight", "invoice_no", "coa_no", "pod_reference", "remarks"].forEach(function (key) { if (details[key] !== undefined && details[key] !== "") patch[key] = details[key]; });
    ...
    var eventPayload = { trip_id: trip.id, trip_no: trip.trip_no, vehicle_number: trip.vehicle_number, stage_key: stage, stage_label: target[0], stage_sequence: target[2], planned_time: planned, actual_time: actual, duration_minutes: minutes(previous, actual), delay_minutes: Math.round((new Date(actual) - new Date(planned)) / 60000) };
  ...
  window.BulkerAPI = { configured: configured, hasSession: hasSession, signIn: signIn, signOut: signOut, getData: getData, createOrder: createOrder, addVehicle: addVehicle, createSchedule: createSchedule, completeStage: completeStage, nextStage: nextStage, stages: STAGES, targets: TARGETS };
...
create table if not exists orders (
 ...
 order_date date not null default current_date, party_name text not null, item_name text not null default 'Maida',
...
create table if not exists trips (
 ...
 empty_weight numeric, loaded_weight numeric, net_weight numeric, invoice_no text, coa_no text, pod_reference text, remarks text,
 created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create table if not exists stage_events (
 ...
 planned_time timestamptz, actual_time timestamptz, duration_minutes int, delay_minutes int,
